...
namespace events
{
OnEndOfLine::OnEndOfLine(std::weak_ptr<LineBuffer> line_buffer) noexcept
  : buffer_ref_{std::move(line_buffer)}
{
}

void OnEndOfLine::operator()() noexcept
{
...
std::string OnEndOfLine::events_get() const noexcept
{
